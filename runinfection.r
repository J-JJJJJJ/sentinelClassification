...
monteCarloSirListCont <- function(graph, nRuns) {
  ...
  infectedGraphs <-
    c(infectedGraphs1, infectedGraphs2, infectedGraphs3)
  tableList <-
    ldply(infectedGraphs, igraph::as_data_frame, what = "vertices")
...
# get the file names
graphs <-
  list.files("graphs with prop/", pattern = "*.xml", full.names = TRUE)
...
for (g in graphList) {
  ...
  infectionTable <-
    infectionTable %>% dplyr::select(-dplyr::one_of(dropColumns)) %>% mutate(Iduration =
                                                                               SIduration - Sduration)
  ...
  # remove columns that are no longer interesting
  dropColumns <-
    c("SIduration", "maxTime", "percentageInfected", "Iduration")
  infectionTable <-
    infectionTable %>% dplyr::select(-dplyr::one_of(dropColumns))
  
  infectionTable <- infectionTable %>%
    group_by_at(names(infectionTable)[-grep("Sduration", names(infectionTable))]) %>%
    dplyr::summarise(
      sentinel = Sduration %>% mean() %>% round(5),
      sentinelVar = var(Sduration) %>% round(5)
    )
