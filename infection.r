...
runContSIR<-function(graph, tau){
  ...
  while(length(V(graph)[V(graph)$currentState==1])>0 ){ 
    ...
    neighbourList<-unlist(adjacent_vertices(graph, V(graph)[V(graph)$currentState==1]), FALSE, FALSE)
    neighbourList<-neighbourList[!V(graph)[neighbourList]$wasInfected]
  ...
  percentageInfected<-round(100*sum(V(graph)$wasInfected)/vcount(graph), digits = 2)
  graph<-set_vertex_attr(graph, "percentageInfected", value = percentageInfected)
