...
improveEdgelist <- function(oldEdges, clusters) {
  ...
  oldEdges$community1 <-
    left_join(oldEdges, clusters, by = c("V1" = "node"))$community
  oldEdges$community2 <-
    left_join(oldEdges, clusters, by = c("V2" = "node"))$community
  
  # add inter / intra community booleans
  oldEdges <-
    oldEdges %>%  mutate(intercommunity = case_when((community1 == community2) ~
                                                      FALSE, TRUE ~ TRUE))
  oldEdges <-
    oldEdges %>%  mutate(intracommunity = case_when((community1 == community2) ~
                                                      TRUE, TRUE ~ FALSE))
...
original <- function(graphName, oldEdges, clusters) {
  ...
  # Create the graph
  graph <-
    oldEdges %>% select(V1, V2, intercommunity, intracommunity) %>% graph_from_data_frame(directed = FALSE)
  # create a data frame of the vectices (one vector)
  vertex <-
    graph %>% vertex_attr("name") %>% as.integer() %>% data.frame()
  names(vertex) <- "vertex"
  
  # create the community vector
  community <-
    left_join(vertex, clusters, by = c("vertex" = "node"))$community
...
HCM <- function(graphName, oldEdges, clusters, version) {
  ...
  inter <-
    oldEdges %>% dplyr::filter(intercommunity) %>% select(V1, V2, community1, community2)
  oldEdges <-
    oldEdges %>% dplyr::filter(intracommunity) %>% select(V1, V2)
...
HCMStar <- function(graphName, oldEdges, clusters, version) {
  ...
  inter <-
    oldEdges %>% dplyr::filter(intercommunity) %>% select(V1, V2 , community1, community2)
  intra <-
    oldEdges %>% dplyr::filter(intracommunity) %>% select(V1, V2, community1)
  ...
  oldEdges <-
    foreach(i = unique(intra$community1), .combine = rbind) %dopar% {
      source("graphFunctions.r")
...
writeGraph <- function(graph) {
  ...
  path <-
    paste0("graphs without prop/", name, "_", type, version, ".xml")
...
erdosModel <- function(n) {
  ...
  graph <-
    set_vertex_attr(graph, "graphName", value = paste0("erdosrenyi", n))
...
for (i in 1:18) {
  ...
  name <-
    sub(pattern = "(.*)\\..*$", replacement = "\\1", basename(name))
