...
if(!exists("smallTable")){
  ...
  smallTable<-list.files("infectionresult/", pattern="*.csv", full.names=TRUE)
  ...
  smallTable <- foreach(i= 1:length(smallTable)) %dopar% {
    library(data.table)
    table = fread(smallTable[[i]], stringsAsFactors=TRUE, na.strings=c("NA", "", NULL))
    table}
  stopCluster(cl)
}
...
smallTable<-smallTable %>% 
  ...
  dplyr::mutate( sentClass = case_when(rank(sentinel, ties.method="random")<= ceiling(0.15*n()) ~T, T~F )) %>%
  ungroup()
