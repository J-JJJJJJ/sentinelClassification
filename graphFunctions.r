...
rewire<-function(edges, inter){
  ...
  if(nrow(edges)<4){return(edges %>% select(V1, V2))}
  ...
  for(i in 1:nruns){
    ...
    if(sample(c(T,F), 1)){
      ...
      if(inter){sample1<-sample1 %>%dplyr::rename(community1=community2, community2=community1)}
    }
    ...
    if(sample(c(T,F), 1)){
      ...
      # if inter community, also swap the community columns
      if(inter){sample2<-sample2 %>%dplyr::rename(community1=community2, community2=community1)}
    }
    ...
    test<-edges %>% dplyr::filter(   (V1==sample$V1[1] && V2==sample$V2[1]) ||
                              (V1==sample$V1[2] && V2==sample$V2[2]) ||
                              (V1==sample$V2[1] && V2==sample$V1[1]) ||
                              (V1==sample$V2[2] && V2==sample$V1[2])
                              )
    if(nrow(test)>0){next}
    ...
    if(inter){
      ...
      sample<-sample %>% dplyr::mutate(community2=rev(community2))

      #if(nrow(sample %>% filter(community1 == community2))>0 ){next}
      sample<-sample %>% dplyr::mutate(community3=community1-community2)
      if(0 %in% sample$community3){next}
      
      
      # remove old edges
      edges<-edges%>%filter(!index %in% sample$index)
      # add the new edges
      sample<-sample %>% dplyr::select(V1, V2, community1, community2, index)
